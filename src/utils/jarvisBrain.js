...
const ACTIVE_GEMINI_MODELS = [
  'gemini-flash-lite-latest', // ~600ms (Ultra-fast, high-quota)
  'gemini-3.5-flash-lite',    // ~1100ms
  'gemini-3.6-flash'          // ~2000ms
];
...
6. **BÚSQUEDAS Y DISPONIBILIDAD DE OBRAS (ACCIÓN PROACTIVA OBLIGATORIA)**:
   - Cuando la persona pregunte por disponibilidad de cuadros, personajes, series, autos, películas o anime (ej: "¿tienen de Spider-Man?", "¿tienes a Goku?", "¿qué cuadros de autos tienen?", "¿hay algo de Van Gogh?"):
     -> **NUNCA te limites a decir solo "sí, tenemos cuadros" para esperar a que te pidan verlos**.
     -> **DEBES INVOCAR SIEMPRE E INMEDIATAMENTE** la herramienta 'recomendar_obras' con los IDs de las obras en ese mismo mensaje para que aparezcan las tarjetas interactivas de inmediato.
...
══════════════════════════════════════════════════════════
INVENTARIO COMPLETO OFICIAL DE OBRAS DISPONIBLES:
══════════════════════════════════════════════════════════
${postersIndex}
`;
}
  ...
  for (const modelName of ACTIVE_GEMINI_MODELS) {
    try {
      ...
      const timeoutPromise = new Promise((_, reject) =>
        setTimeout(() => reject(new Error('TIMEOUT_4.5S')), 4500)
      );
      ...
      if (!executedActions.some(a => a.type === 'catalog_matches') && !executedActions.some(a => a.type === 'custom_quote')) {
        ...
        if (matched.length > 0) {
          ...
          if (onExecuteTool && typeof onExecuteTool === 'function') {
            try { onExecuteTool(autoAction.type, autoAction); } catch (e) {}
          }
        }
      }
